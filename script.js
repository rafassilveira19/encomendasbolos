...
const firebaseConfig = {
  ...
  pedidosCollection.orderBy('data').onSnapshot((snapshot) => {
  ...
  function atualizarTabelasFirebase(pedidosData) {
    ...
    const pedidosAtivos = pedidosData.filter(p => !p.entregue).sort((a,b)=>compararData(a.data,b.data));
    ...
    for (const data of Object.keys(pedidosPorData).sort((a,b)=>compararData(a,b))) {
  ...
  function atualizarFinalizadosFirebase(pedidosData) {
    ...
    const finalizados = pedidosData.filter(p => p.entregue).sort((a,b)=>compararData(a.data,b.data));
  ...
  btnImprimir.addEventListener('click', () => {
    ...
    pedidosCollection.orderBy('data').get().then(snapshot => {
      const pedidosAtivos = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }))
        .filter(p => !p.entregue)
        .sort((a,b)=>compararData(a.data,b.data));
